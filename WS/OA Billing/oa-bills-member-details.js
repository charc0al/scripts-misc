divs = document.getElementById('mainform:memberDetailsTabId').childNodes[0].childNodes[0].childNodes[0].childNodes[0].childNodes;
for(i=0;i<divs.length;i++){if(divs[i].style.display!='none'){displayedDiv=divs[i];}}
var memberNo='';
var tableIndex=0;
...
for(i=2;i<members.length-2;i++)
{
	...
	if((cols == headers.length) && (members[i].innerText != ''))
	{
		...
		benefits = members[i].childNodes[2].getElementsByTagName('table')[tableIndex].getElementsByTagName('tr');
	...
	if(members[i].childNodes.length > 0){
		...
		if((label == 'Total')||(label == 'Subtotal Current')){
